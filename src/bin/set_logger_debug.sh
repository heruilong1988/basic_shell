...
echo "xxx operation tool - set particular method's or parameter's logger level debug."echo `date +"%Y-%m-%d %H:%M:%S"`
...
if [[ $1 == "-m" && $# -eq 3 ]]; then
    SERVER_IP_PORT=$2
    METHOD_NAME=$3
    /usr/bin/curl http://${SERVER_IP_PORT}/jolokia/exec/xxx:name=LoggerControllerJmx/putMethodMapping/${METHOD_NAME}
    exit 1
elif [[ $1 == "-m" ]]; then
...
if [[ $1 == "-p" && $# -eq 4 ]]; then
    ...
    PARA_NAME=$3
    PARA_VALUE=$4
    /usr/bin/curl http://${SERVER_IP_PORT}/jolokia/exec/xxx:name=LoggerControllerJmx/putParameterMapping/${PARA_NAME}/${PARA_VALUE}
elif [[ $1 == "-p" ]]; then
    SERVER_IP_PORT=localhost:8778
    PARA_NAME=$2
    PARA_VALUE=$3
